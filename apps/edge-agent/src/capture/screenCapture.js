...
export class ScreenCapture {
    ...
    constructor() {
        ...
        this.worker = null; // Using any due to Tesseract.js type issues
        this.OCR_ENGINE = 'eng';
        this.MIN_CAPTURE_INTERVAL = 1000; // 1 second
     ...
     */
    updateApplicationContext(metadata, textContent) {
    ...
    async initializeOCR() {
        try {
            ...
            if (this.worker.logger) {
                // @ts-ignore
                this.worker.logger = (m) => console.log(`Tesseract: ${m.status}`);
            }
            // Set error handler
            // @ts-ignore - Tesseract.js worker has an errorHandler property in runtime
            this.worker.errorHandler = (err) => console.error('Tesseract error:', err);
     ...
     */
    async startCapture(interval = 5000, mainWindow) {
    ...
    async getSources(options) {
        try {
            ...
            const sources = await desktopCapturer.getSources({
                types: (defaultOptions.types || ['window']),
    ...
    async captureActiveWindow() {
        ...
        try {
            ...
            if (targetWindow) {
                ...
                source = sources.find(s => s.id === `window:${sourceId}` ||
                    s.name === targetWindow.getTitle()) || source;
            }
