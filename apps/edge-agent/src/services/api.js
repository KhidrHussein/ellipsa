...
export class ProcessorClient extends ServiceClient {
    ...
    async processScreenshot(imageData, metadata) {
        return this.request({
            method: 'POST',
            url: '/process/image',
            data: {
...
export async function initializeServices() {
    ...
    const results = await Promise.allSettled(services.map(({ name, client, url }) => client.makeRequest({ method: 'GET', url: '/health' })
        .then(() => ({ name, url, status: 'fulfilled' }))
        .catch(error => ({ name, url, status: 'rejected', error }))));
    const failedServices = results.filter((result) => result.status === 'fulfilled' && result.value.status === 'rejected');
    ...
    const successfulServices = results.filter((result) => result.status === 'fulfilled' && result.value.status === 'fulfilled');
